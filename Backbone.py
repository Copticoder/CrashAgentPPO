...
class ActorCriticBackbone(tf.keras.Model):
    def __init__(self):
        ...
        # self.backbone.add(tf.keras.Input(shape=input_shape))
        self.backbone.add(tf.keras.layers.Conv2D(filters = 32,kernel_size=(3,3),activation='relu'))
        self.backbone.add(tf.keras.layers.BatchNormalization())
        self.backbone.add(tf.keras.layers.Conv2D(filters = 64,kernel_size=(4,4),activation='relu'))
        self.backbone.add(tf.keras.layers.BatchNormalization())
        self.backbone.add(tf.keras.layers.Conv2D(filters = 64 ,kernel_size=(6,6),activation='relu'))
